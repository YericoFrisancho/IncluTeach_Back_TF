...
public class ForoQueSigueUsuarioController {
    ...
    @Autowired
    private ForoQueSigueUsuarioRepository foroSeguidoRepository;
    ...
    @DeleteMapping("/dejar-seguir/usuario/{uid}/foro/{fid}")
    public ResponseEntity<Void> dejarDeSeguir(@PathVariable Integer uid, @PathVariable Integer fid) {
        foroSeguidoRepository.deleteByUsuarioIdAndForoId(uid, fid);
        return ResponseEntity.noContent().build();
    }
}
